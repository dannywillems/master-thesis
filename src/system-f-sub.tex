...
contexte $\Gamma$ dans les règles de sous-typage.
...
\begin{mathpar}
  ...
  {\Gamma, x : T_{1} \vdash \lambda (x : T_{1}) t : T_{1} \rightarrow T_{2}}
  \quad (\textsc{T-ABS})
  \and
  \inferrule
...
\begin{mathpar}
  ...
  \and
  \inferrule
  {\Gamma, X <: U_{1} \vdash S_{2} <: T_{2}}
  {\Gamma \vdash \forall X <: U_{1}. S_{2} <: \forall X <: U_{1}. T_{2}}
  \quad (\textsc{S-ALL})
\end{mathpar}
