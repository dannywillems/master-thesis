...
T_{1} = \left\{ t : S .. T \right\}$ et $T_{1} = T_{2} = \left\{ a :
  U\right\}$. La question  $S_{1} \wedge S_{2} <: T_{1} \wedge T_{2}$ est alors
vraie par réflexivité ou encore par l'arbre de dérivation suivant :
...
L'algorithme travaille sur la structure de $S$ et $T$. Voici, dans l'ordre,
comment l'algorithme gère les différents cas.
...
Même cas que précedemment en utilisant la règle (ALL <: ALL).
...
\begin{mathpar}
  \inferrule*[Left=(<: AND)]
  {
    ...
    {
      \inferrule*[Left=($AND_{1}$<: et $AND_{2}$ <:)]
