...
Avant de discuter des algorithmes de sous-typage, nous allons discuter d'un
autre algorithme important qui sera utilisé à plusieurs reprises.

Un problème qui se pose lors de l'écriture des algorithmes de sous-typage et de
typage, est : étant donnés une variable $x$ et un contexte, quelle est la borne
inférieure ou la borne supérieure du type dépendant $x.A$ ? La question se pose,
par exemple, quand nous devons comparer deux types dépendants $x.A$ et $y.A'$.
...
\item[$\bullet$] Si $T = \mu(y : S^{y})$, nous appelons récursivement l'algorithme sur
  $S^{x}$ après avoir substitué $y$ par $x$ dans $S$ grâce à la règle
  (VAR-PACK).\footnote{Notons que la variable est importante dans ce cas, ce qui
  n'a pas été directement évident.}
\item[$\bullet$] Si $T = T_{1} \wedge T_{2}$, nous
  appliquons l'algorithme récursivement sur $T_{1}$ et sur $T_{2}$. Notons
  $T'_{1}$ et $T'_{2}$ les résultats. Plusieurs cas sont possibles :
  \begin{itemize}
...
\begin{itemize}
\item[$\bullet$] Si $d = \left\{ A = S \right\}$ et $x : T$, nous renvoyons $T \wedge
  \left\{ A : S .. S \right\}$.
\item[$\bullet$] Si $d = \left\{ a = t \right\}$ et $x : T$, nous utilisons l'algorithme de
  typage sur $t$ et le contexte $\Gamma, x : T$ et notons $S$ le résultat. Nous
  renvoyons $T \wedge \left\{ a : S \right\}$.
\item[$\bullet$] Si $d = d_{1} \wedge d_{2}$ et $x : T$, nous appelons récursivement
...
où la règle (<:-AND) est utilisée en premier suivie de (AND-1-<:) et (AND-2-<:) sur chaque
branche. Cependant, si nous utilisions (AND-1-<:) (ou (AND-2-<:)) en premier,
nous serions incapables d'arriver au résultat voulu. Cet exemple montre qu'il
est important d'envisager les différents arbres de dérivation possibles.
...
Aucun document sur DOT ne décrit actuellement un algorithme
de sous-typage pour DOT, le développement d'un tel algorithme à partir de règles
...
Il n'a pas été démontré que l'algorithme donné ci-dessous est complet et
correct\footnote{Et il ne doit certainement pas l'être en raison de la
  transitivité et de l'algorithme best\_bound qui n'est ni complet ni correct.}.
L'algorithme est cependant satisfaisant sur des exemples concrets. Diverses
améliorations et travaux en cours sont listés à la fin de ce chapitre.
...
Dans l'implémentation OCaml, les variables $x$ sont deux atomes différents. Un
nouvel atome $x'$ est généré et un renommage de $x$ par $x'$ est réalisé dans
$T_{1}$ et $T_{2}$. La variable $x'$ est alors ajoutée avec le type $S_{2}$ dans
le contexte. L'appel récursif est effectué avec les types $T_{1}$ et $T_{2}$
dans lesquels la variable $x$ a été substituée par la variable $x'$.
...
ci-dessous.}. En effet, les règles (S-REFL), (SEL <:) et (<: SEL) peuvent être
employées.

L'algorithme procède par cas :
\begin{itemize}
\item[$\bullet$] Dans le cas de la réflexivité, cela signifie que $x$ et $y$ sont les mêmes
...
\item[$\bullet$] Sinon, nous testons en premier (SEL <:). Nous ajoutons en plus (T-SUB) et
...
De plus, dans le cas (<: AND), nous devons gérer séparément et en premier les types récursifs à cause de
...
La régle de réflexivité (S-REFL) n'est pas implémentée directement. En effet,
...
faudrait trouver un type $U$ tel que $S <: U$ et $U <: T$, ce qui n'est pas
...
correspondant étant $\nu(x) d$ défini précédemment. Dans le langage de surface, les enregistrements
