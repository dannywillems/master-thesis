...
\begin{proof}
  ...
  \item $v$ est de type $\left\{ l_{i} : T_{i} \right\}^{1 \leq i \leq n}$. Les
    seules règles possibles sont (T-SUB) et (T-RCD). Même argument que
    précédemment pour (T-SUB) ainsi que pour (T-RCD).
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \item (T-RCD). Alors $t = \left\{ l_{i} = t_{i} \right\}^{1 \leq i \leq n}$ et
    ...
    \item s'il existe $i$ tel que $t_{i}$ n'est pas une valeur, alors nous
      utilisons l'hypothèse de récurrence et (E-RCD) s'applique.
    \end{itemize}
  \item (T-PROJ). Nous avons alors $t = t_{1}.l_{j}$ avec $T = \left\{ l_{i} : T_{i} \right\}^{1
    \leq i \leq n}$
  et $t_{i} : T_{i}$.

  Par hypothèse de récurrence, $t_{1}$ est soit une valeur, soit il existe
  $t_{1}'$ tel que $t_{1} \rightarrow t_{1}'$. Si $t_{1}$ est une valeur, alors
  le lemme des formes canoniques, nous utilisons le lemme des formes canoniques
