...
%%% TODO: best_bound peut boucler quand on a x.A. On peut régler le problème est en se
%%% souvenant des questions déjà posées. Si le label n'est pas le même, on
%%% renvoit None. Sinon, on renvoie le type de x.
...
Nous utilisons la même technique que précédemment, c'est-à-dire l'induction
...
%% TODO: En fait, il faudrait montrer que si pour le codage c'est vrai, alors
%% c'est vrai dans F-SUB. Cependant, on n'est pas sûr que pour la transitivité
%% ce soit bon. On devrait montrer que la transitivité peut-être push back dans
%% la preuve.
%% Historique : https://en.wikipedia.org/wiki/CLU_(programming_language)
% Algorithme : finit toujours.
%% Semi algorithme : finit pas toujours. Donc ici c'est semi algo.

%\section{Notion bonne formation}
